...
get_socket()
{
	...
	hints.ai_socktype =
	    SOCK_STREAM; // We want to use TCP to ensure it gets there
	int return_value =
	    getaddrinfo(SERVER, SERVERPORT, &hints, &server_info);
	if (return_value != 0) {
	...
	// We end up with a linked-list of addresses, and we want to connect to
	// the first one we can
	for (p = server_info; p != NULL; p = p->ai_next) {
		// Try to make a socket with this one.
		if ((sockfd = socket(p->ai_family, p->ai_socktype,
			 p->ai_protocol)) == -1) {
			// Something went wrong getting this socket, so we can
			// try the next one.
		...
		if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
			// If something went wrong connecting to this socket, we
			// can close it and move on to the next one.
			close(sockfd);
			perror("client: connect");
			continue;
		}

		// If we've made it this far, we have a valid socket and can
		// stop iterating through.
		break;
	}

	// If we haven't gotten a valid sockaddr here, that means we can't
	// connect.
	if (p == NULL) {
...
is_prime_rpc(int number)
{
	...
	// Now, we can grab a socket we can use to connect see how we can
	// connect
	...
	int buf[1]; // Just receiving a single byte back that represents a
		    // boolean.
	...
	// Since we just have the one byte, we don't really need to do anything
	// while unpacking it, since one byte in reverse order is still just a
	// byte.
	bool result = buf[0];
