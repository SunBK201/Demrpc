...
get_and_bind_socket()
{
	...
	for (p = server_info; p != NULL; p = p->ai_next) {
		...
		// We want to be able to reuse this, so we can set the socket
		// option.
		int yes = 1;
		...
		if (bind(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
			// If something went wrong binding this socket, we can
...
main(void)
{
	...
	while (1) {
		...
		// Once we've accepted an incoming request, we can read from it
		// into a buffer.
		int buffer;
		...
		if (send(new_fd, &number_is_prime, sizeof number_is_prime, 0) ==
		    -1) {
			perror("send");
		}
		close(new_fd);
